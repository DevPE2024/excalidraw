...
const FloatingAiAssistant = () => {
  ...
  const handleSend = async () => {
    console.log('🔥🔥🔥 FLOATING AI: handleSend CHAMADO!!! 🔥🔥🔥');
    console.log('🔥🔥🔥 Mensagem:', message);
    if (message.trim()) {
      console.log('🔥🔥🔥 Mensagem não está vazia, continuando...');
      ...
      try {
        ...
        const response = await fetch("http://localhost:8001/api/external/jazzup/use-credit", {
          method: "POST",
          headers: {
        ...
        if (response.status === 402) {
          ...
          alert(`Créditos insuficientes! Você precisa de ${data.required} ponto(s), mas tem apenas ${data.remaining}. Faça upgrade do seu plano no Prodify.`);
          return;
        }
        ...
        console.log(`✅ ${data.pointsUsed} ponto(s) consumido(s). Pontos restantes: ${data.remaining}`);
        console.log('Sending message:', message);
        
        // Aqui você implementaria a lógica real de envio para a IA
